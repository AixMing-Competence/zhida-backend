...
import com.aixming.yudada.mapper.ScoringResultMapper;
import com.aixming.yudada.model.dto.scoringResult.ScoringResultQueryRequest;
...
import com.aixming.yudada.model.vo.UserVO;
...
import com.aixming.yudada.utils.SqlUtils;
...
public class ScoringResultServiceImpl extends ServiceImpl<ScoringResultMapper, ScoringResult> implements ScoringResultService {
    ...
    public void validScoringResult(ScoringResult scoringResult, boolean add) {
        ...
        if (add) {
            // 补充校验规则
            ThrowUtils.throwIf(StringUtils.isBlank(resultName), ErrorCode.PARAMS_ERROR, "应用名称不能为空");
            ThrowUtils.throwIf(appId == null || appId <= 0, ErrorCode.PARAMS_ERROR, "appId 非法");
        }
        // 修改数据时，有参数则校验
        // 补充校验规则
        if (StringUtils.isNotBlank(resultName)) {
            ThrowUtils.throwIf(resultName.length() > 80, ErrorCode.PARAMS_ERROR, "应用名称长度大于 80");
        }
        if (appId != null) {
